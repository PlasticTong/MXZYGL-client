#include "modeltablelabel.h"

modeltablelabel::modeltablelabel(uint panel_rows, uint panel_cols, QWidget *parent) : QWidget(parent)
{
...
void modeltablelabel::doItemSelectionChanged()
{
            ...
            for(int r = 0, k = 0; r < num_rows; ++r)
            {
                for(int c = 0; c < num_cols; ++c, ++k, ++i)
                {
                    if(k < span)
                    {
                        ...
                        txtInfo += " 标签:" + _tablewidget->item(i, 3)->text();
...
void modeltablelabel::doVerticalSectionClicked(int idx)
{
...
void modeltablelabel::doVerticalSectionDoubleClicked(int idx)
{
    qDebug() << "doVerticalSectionDoubleClicked " << idx;
}

void modeltablelabel::setModels(QList<ModelInfo> &models)
{
...
void modeltablelabel::setModels3(const QJsonObject &table_data)
{
    _tablewidget->clear();
...
void modeltablelabel::setModels4(const QJsonObject &models)
{
    _models = models;
    this->refresh();
}

void modeltablelabel::readFiles(const QStringList &filenames)
{
    // 读取文件并显示
    _models["titles"] = QJsonArray({"name", "category", "tags", "description", "hash"});
...
TableJson modeltablelabel::getModels()
{
//    QJsonObject models;
//    // 获取表格内容
